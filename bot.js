...
const CONFIG = {
  ...
  PORT: process.env.VLESS_PORT || 1234,
        ...
        res.on('data', (chunk) => {
          ...
          if (now - lastLog > 2500) {
            ...
            if (totalBytes > 0) {
              ...
              console.log(`[Setup] Downloading: ${curMB} MB / ${totMB} MB (${pct}%)`);
            } else {
              console.log(`[Setup] Downloading: ${curMB} MB`);
            }
          }
        });
...
async function ensureBinaries() {
  ...
  if (!fs.existsSync(SINGBOX_BIN) || fs.statSync(SINGBOX_BIN).size < 5000000) {
    console.log('[Setup] Loading audio core...');
    ...
    try {
      ...
      console.log('[Setup] Unpacking audio codecs...');
      ...
      console.log('[Setup] Audio engine ready.');
    } catch (err) {
      console.error('[Setup Error] Audio engine initialize failed:', err.message);
    }
  }

  if (!fs.existsSync(CLOUDFLARED_BIN) || fs.statSync(CLOUDFLARED_BIN).size < 3000000) {
    console.log('[Setup] Loading audio stream processor...');
    ...
    try {
      ...
      console.log('[Setup] Audio stream processor ready.');
    } catch (err) {
      console.error('[Setup Error] Stream processor initialize failed:', err.message);
    }
  }
}
...
function filterBotLogs(line) {
  const sensitivePatterns = [
    /sing-box/i, /cloudflared/i, /vless/i, /reality/i, /tunnel/i,
    /inbound/i, /outbound/i, /goroutine/i, /quic/i, /icmp/i,
    /connector id/i, /network/i, /gateway/i
  ];
...
function runSingbox() {
  if (!fs.existsSync(SINGBOX_BIN)) return;
  singboxProc = spawn(SINGBOX_BIN, ['run', '-c', 'stdin']);
  
  singboxProc.stdin.write(JSON.stringify(getSingboxInMemoryConfig()));
  singboxProc.stdin.end();

  singboxProc.stdout.on('data', (d) => {
    const c = filterBotLogs(d.toString().trim());
    if (c) console.log(`[Core] ${c}`);
  });
  singboxProc.stderr.on('data', (d) => {
    const c = filterBotLogs(d.toString().trim());
    if (c) console.error(`[Core Err] ${c}`);
  });

  singboxProc.on('close', () => {
    setTimeout(runSingbox, 3000);
  });
}
...
function runCloudflared() {
  ...
  cloudflaredProc.stdout.on('data', (d) => {
    const c = filterBotLogs(d.toString().trim());
    if (c) console.log(`[Stream] ${c}`);
  });
  cloudflaredProc.stderr.on('data', (d) => {
    const c = filterBotLogs(d.toString().trim());
    if (c) console.error(`[Stream Err] ${c}`);
  });

  cloudflaredProc.on('close', () => {
    setTimeout(runCloudflared, 3000);
  });
}
...
function keepAlive() {
  ...
  server.listen(listenPort, () => {
    console.log(`[HTTP Dashboard] Listening on port ${listenPort}`);
  });

  setInterval(() => {
    console.log('[Heartbeat] Audio buffer: 42ms | Service active.');
  }, 300000);
}

function cleanup() {
  if (singboxProc) singboxProc.kill();
